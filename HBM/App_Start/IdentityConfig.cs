...
namespace HBM
{
    ...
    public class ApplicationUserManager : UserManager<HBMUser>
    {
        public ApplicationUserManager(IUserStore<HBMUser> store)
            : base(store)
        {
        }
        ...
        {
            var manager = new ApplicationUserManager(new UserStore<HBMUser>(context.Get<HBMContext>()));
            // Konfiguruj logikę weryfikacji nazw użytkowników
            manager.UserValidator = new UserValidator<HBMUser>(manager)
            {
            ...
            manager.RegisterTwoFactorProvider("Kod — telefon", new PhoneNumberTokenProvider<HBMUser>
            {
                MessageFormat = "Twój kod zabezpieczający: {0}"
            });
            manager.RegisterTwoFactorProvider("Kod — e-mail", new EmailTokenProvider<HBMUser>
            {
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider = 
                    new DataProtectorTokenProvider<HBMUser>(dataProtectionProvider.Create("ASP.NET Identity"));
            }
            return manager;
        }
    }
    ...
    public class ApplicationSignInManager : SignInManager<HBMUser, string>
    {
        ...
        public override Task<ClaimsIdentity> CreateUserIdentityAsync(HBMUser user)
        {
